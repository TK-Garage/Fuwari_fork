...
final class HotKeyManager: NSObject {
    ...
    private(set) lazy var ocrCaptureKeyCombo: KeyCombo? = {
        return self.defaults.archiveDataForKey(KeyCombo.self, key: Constants.UserDefaults.ocrCaptureKeyCombo)
    }()
    private(set) var ocrCaptureHotKey: HotKey?

    func configure() {
        registerHotKey(keyCombo: captureKeyCombo)
        registerOCRHotKey(keyCombo: ocrCaptureKeyCombo)
    }
}
...
extension HotKeyManager {
    ...
    func registerOCRHotKey(keyCombo: KeyCombo?) {
        ...
        HotKeyCenter.shared.unregisterHotKey(with: Constants.HotKey.ocrCapture)
        ocrCaptureKeyCombo = keyCombo
        guard let keyCombo = keyCombo else {
            ocrCaptureHotKey = nil
            MenuManager.shared.updateOCRCaptureMenuItem()
            return
        }

        let hotKey = HotKey(identifier: Constants.HotKey.ocrCapture, keyCombo: keyCombo, target: AppDelegate(), action: #selector(AppDelegate.captureForOCR))
        hotKey.register()
        ocrCaptureHotKey = hotKey
    ...
    private func saveOCRKeyCombo(keyCombo: KeyCombo?) {
        if let keyCombo = keyCombo {
            defaults.setArchiveData(keyCombo, forKey: Constants.UserDefaults.ocrCaptureKeyCombo)
        } else {
            defaults.removeObject(forKey: Constants.UserDefaults.ocrCaptureKeyCombo)
        }
    }
}
